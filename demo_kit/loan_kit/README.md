...
### Application-stage bundles (aligned with the processing prompts)

The bundles above are *servicing* files (sanction letter, EMI schedule, loan
statement) that show the deterministic reconciliation. These six are
*application* files — what a branch actually receives with a new loan
request — written against the eligibility criteria in each product's
processing prompt (Settings → Processing prompts). Each is a different
fictional applicant; the positive bundle satisfies every criterion, the
negative one seeds named failures (see its `README.txt`). They are the same
documents the email-intake sample packs use (`samples/build_email_packs.py`)
and are generated by `application_docs.py` + `application_kit.py`, called
from `build_kit.py`.

| Folder | Box | Docs | Verdict | Applicant — highlights |
|---|---|---|---|---|
| `home_loan_application_positive` | Home | 6 | passes | Rahul Sharma — age 38, 58 at maturity; 6 months of stable salary credits matching slips; FOIR 36.7% (EMI 52,069.39 / net 142,000.00) |
| `home_loan_application_negative` | Home | 6 | fails | Deepak Rao — FOIR 121.8% (EMI 69,425.86 / net 57,000.00) - above 50%; LTV 95.2% - above 80% |
| `vehicle_loan_application_positive` | Vehicle | 6 | passes | Priya Menon — age 33; licence valid till 2035; EMI-to-income 20.2% (EMI 25,202.23 / net 125,000.00) |
| `vehicle_loan_application_negative` | Vehicle | 6 | fails | Arjun Iyer — driving licence EXPIRED 02-02-2026; EMI-to-income 144.4% - above 45% |
| `personal_loan_application_positive` | Personal | 5 | passes | Sneha Kulkarni — age 34; salaried 6 years; EMI-to-income 14.4% (EMI 16,846.98 / net 117,000.00) |
| `personal_loan_application_negative` | Personal | 5 | fails | Vikram Nair — EMI-to-income 97.8% including existing EMIs - above 40%; credit score 688 with a 45-day delinquency in 05-2026 |

Only the salary slip reconciles in code here; the application form, identity,
property, bank-statement and credit-report documents go to the model, so
expect ~85–100% AI share, ~30k tokens and 50–100 s per bundle.
