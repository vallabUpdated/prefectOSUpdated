...
// The server hands back a resolved path; it only ever has to be encoded.
function docHref(path, download = false) {
  return `/loan/document?path=${encodeURIComponent(path)}`
    + (download ? "&download=1" : "");
}
...
// "+ Sample Preset" points at the demo kit shipped in demo_kit/loan_kit —
// one bundle per box, so the button always lands on documents that exist.
// The paths are relative to the server's working directory (the repo root),
// and the bundles are regenerated by demo_kit/loan_kit/build_kit.py.
// Each product presets to its NEGATIVE bundle — the run that shows the engine
// catching something. The matching _positive bundle sits beside it for the
// straight-through half of a demo. Reports go to a sibling output/ folder, so
// a demo's artifacts stay next to the documents that produced them.
const SAMPLE_ROOT = "./demo_kit/loan_kit";
const SAMPLE_BUNDLE = {
  home: "home_loan_negative",
  vehicle: "vehicle_loan_negative",
  mortgage: "mortgage_loan_negative",
  personal: "personal_loan_negative",
  statement: "account_statement_csv",
  account_statement: "account_statement_csv",
  kyc: "kyc_set",
  general: "general_mixed",
};
...
export default function LoanCard({ box, actions, fxRate, policyPath = "" }) {
  ...
  const handleUsePreset = async () => {
    const bundle = SAMPLE_BUNDLE[box.loanType] || "home_loan_positive";
    actions.setField(box.loanType, "inputPath",
                     `${SAMPLE_ROOT}/bundles/${bundle}`);
    actions.setField(box.loanType, "outputPath",
                     `${SAMPLE_ROOT}/output/${box.loanType}`);
    // Create the output folder now, so "📂 Browse…" can open it and the
    // operator can see where the reports will land before starting the run.
    // Best-effort: if this fails the run still creates the folder itself.
    try {
      await fetch("/loan/mkdir", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ path: `${SAMPLE_ROOT}/output`, name: box.loanType }),
      });
    } catch {
      /* the server creates the output path when the job starts */
    }
  };
  ...
  return (
    <section className={`loan-card ${themeClass}${active ? " is-active" : ""}`}>
      ...
      <div className="lc-field">
        ...
        {/* The documents themselves — openable before the run, so an operator
            (or a client watching a demo) can read what is about to be
            processed and take a copy away. */}
        {box.scan?.documents?.length > 0 && (
          <details className="lc-docs">
            <summary>
              Documents at this path
              <span className="lc-docs-count">{box.scan.count}</span>
            </summary>
            <ul className="lc-doc-list">
              {box.scan.documents.map((doc) => (
                <li key={doc.path} className="lc-doc">
                  <span className="lc-doc-type">{doc.suffix || "file"}</span>
                  <span className="lc-doc-name" title={doc.path}>{doc.name}</span>
                  <span className="lc-doc-size">{fmtBytes(doc.size)}</span>
                  <a className="lc-doc-link" href={docHref(doc.path)}
                     target="_blank" rel="noreferrer">View</a>
                  <a className="lc-doc-link" href={docHref(doc.path, true)}>
                    Download
                  </a>
                </li>
              ))}
            </ul>
            {box.scan.count > box.scan.documents.length && (
              <div className="lc-hint">
                showing the first {box.scan.documents.length} of {box.scan.count}
              </div>
            )}
            <a className="lc-doc-zip"
               href={`/loan/documents.zip?path=${encodeURIComponent(box.inputPath)}`}>
              ⬇ Download all as .zip
            </a>
          </details>
        )}
      ...
      <div className="lc-field">
        ...
        <div className="lc-mode">
          {[
            ["deterministic", "⚡ Optimized AI", "Reconcile in code; only failures go to the model", "mode-deterministic"],
